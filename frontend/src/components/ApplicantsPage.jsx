...
import { ArrowLeft, CheckCircle, ShieldCheck, Users, XCircle } from 'lucide-react';
...
export const ApplicantsPage = () => {
    ...
    useEffect(() => {
        const fetchApplicantsData = async () => {
            ...
            } catch (error) { console.error("Failed to fetch applicants"); } 
    ...
    const handleApplicantResponse = async (applicantId, status) => {
        try {
            ...
            if (status === 'Approved') navigate('/manage'); 
            else setApplicants(applicants.map(a => a.ApplicantID === applicantId ? { ...a, ApplicationStatus: 'Rejected' } : a));
        } catch (error) { showToast(error.response?.data?.message || "Action failed.", 'error'); }
    };
    ...
    return (
        <div className="py-8 animate-fade-in max-w-4xl mx-auto">
            <button onClick={() => navigate('/manage')} className="mb-6 flex items-center gap-2 text-amber-500 hover:text-amber-400 font-bold uppercase tracking-widest transition-colors cursor-pointer">
            ...
            <div className="bg-[#fdf6e3] p-8 relative shadow-[16px_16px_0px_rgba(0,0,0,0.6)] border-4 border-double border-amber-900">
                <h2 className="text-3xl font-black text-amber-950 mb-2 uppercase tracking-widest flex items-center gap-3"><Users className="text-amber-700" size={32} /> Review Applicants</h2>
                <p className="text-amber-800 mb-8 font-sans font-medium text-lg border-b-2 border-amber-900/20 pb-4">For: <span className="font-bold text-amber-950">{questTitle}</span></p>
                        ...
                        applicants.map(app => (
                            <div key={app.ApplicantID} className={`p-5 border-2 ${app.IsBusy ? 'border-gray-400 bg-gray-200' : 'border-amber-900/30 bg-amber-50'} flex justify-between items-center transition-all`}>
                                <div className="flex items-center gap-4">
                                    <UserAvatar src={app.AvatarUrl} size={40} className="border-2 border-amber-900/50 shadow-sm" />
                                    <div>
                                        <div className="font-bold text-amber-950 text-lg flex items-center gap-2">
                                            {app.FullName}
                                            {checkKycVerified(app.IsKycVerified) ? <ShieldCheck size={18} className="text-emerald-600" title="Tier 1 KYC Verified" /> : null}
