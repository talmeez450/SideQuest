    ...
    return (
        <div className="py-8 animate-fade-in">
            <h1 className="text-4xl font-black text-amber-100 mb-8 tracking-widest uppercase flex items-center gap-3">
                <WalletIcon className="text-amber-500" size={36} /> The Golden Vault
            </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
                <div className="bg-[#fdf6e3] p-6 border-4 border-double border-amber-900 shadow-[8px_8px_0px_rgba(0,0,0,0.6)] flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-amber-900/70 font-bold uppercase tracking-wider text-sm">Available Gold</p>
                        <p className="text-4xl font-black text-amber-950">{walletBalance} G</p>
                    </div>
                </div>
                <div className="bg-slate-800 p-6 border-4 border-double border-slate-600 shadow-[8px_8px_0px_rgba(0,0,0,0.6)] flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-slate-400 font-bold uppercase tracking-wider text-sm">Locked in Escrow</p>
                        <p className="text-4xl font-black text-white">{escrowBalance} G</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-[#fdf6e3] border-4 border-double border-amber-900 shadow-[8px_8px_0px_rgba(0,0,0,0.6)] overflow-x-auto">
                <table className="w-full text-left font-sans">
                    <thead className="bg-amber-900 text-amber-50 uppercase tracking-wider text-xs">
                        <tr>
                            <th className="p-4 py-3 border-b-2 border-amber-950 w-24">Scroll No.</th>
                            <th className="p-4 py-3 border-b-2 border-amber-950">Quest Details</th>
                            <th className="p-4 py-3 border-b-2 border-amber-950">Date</th>
                            <th className="p-4 py-3 border-b-2 border-amber-950 text-right">Amount Flow</th>
                        </tr>
                    </thead>
                                ...
                                <tr key={idx} className="border-b border-amber-900/20 hover:bg-amber-100/50 transition-colors">
                                    ...
                                    <td className="p-4">
                                        ...
                                        <p className="text-xs text-amber-800/80 mt-0.5">
                                            {tx.TransactionType === 'Earned' ? `Hired by ${tx.GiverName}` : `Completed by ${tx.TakerName}`}
                                        </p>
                                    </td>
                                    ...
                                    <td className="p-4 text-right font-black text-lg">
                                        {tx.TransactionType === 'Earned' ? (
                                            <span className="text-green-600 flex items-center justify-end gap-1"><TrendingUp size={18} strokeWidth={3}/> +{tx.RewardAmount}</span>
                                        ) : (
                                            <span className="text-red-600 flex items-center justify-end gap-1"><TrendingDown size={18} strokeWidth={3}/> -{tx.RewardAmount}</span>
                                        )}
                                    </td>
                                </tr>
                            ))
