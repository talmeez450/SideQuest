                ...
                {toasts.map(t => (
                    <div 
                        ...
                        className={`pointer-events-auto flex items-center gap-3 p-4 border-4 border-double shadow-[8px_8px_0px_rgba(0,0,0,0.6)] animate-fade-in transition-all ${
                            t.type === 'success' ? 'bg-green-100 border-green-900 text-green-900' : 'bg-red-100 border-red-900 text-red-900'
                        }`}
                    >
                        ...
                        <button onClick={() => setToasts(prev => prev.filter(toast => toast.id !== t.id))} className="ml-4 hover:scale-110 transition-transform cursor-pointer"><X size={18} /></button>
                    </div>
                ))}
            </div>
